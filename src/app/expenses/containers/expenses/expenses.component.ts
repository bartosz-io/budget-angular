...
export class ExpensesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  openExpenseDialog() {
    this.dialog.open<ExpenseDialogComponent, ExpenseDialogData>(
      ExpenseDialogComponent,
      {
        data: {
  ...
  loadTable$() {
    this.isLoading = true;
    return this.expensesService.getExpenses(this.period).pipe(
      map((data) => {
        this.isLoading = false;
        this.dataSource.data = data;
      })
  ...
  getSubmitExpenseCallback$(): SubmitExpenseCallback {
    return (expense: Expense) =>
      this.expensesService.createOrUpdateExpense(expense).pipe(
