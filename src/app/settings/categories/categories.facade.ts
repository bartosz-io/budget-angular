...
export class CategoriesFacade {
  ...
  loadExpenseCategories() {
    return this.expenseCategoryApi
      .getExpenseCategories()
      .pipe(
        tap((categories) =>
          this.categoriesState.setExpenseCategories(categories)
        )
      );
  }
  ...
  addExpenseCategory(category: ExpenseCategory) {
    ...
    this.expenseCategoryApi.createExpenseCategory(category).subscribe(
      (addedCategoryWithId: ExpenseCategory) =>
        this.categoriesState.updateExpenseCategoryId(
          category,
          addedCategoryWithId
        ),
      (error: any) => {
  ...
  updateExpenseCategory(category: ExpenseCategory) {
    ...
    this.expenseCategoryApi.updateExpenseCategory(category).subscribe(
